...
# gpn = gpus per node
# arg list is:
# script, cfg name, nodes, gpn, mbsz, accum, seq_len, lr, kld,...
exp_list = [
    ...
    ["run_distributed_training.py", "train_lvae_dist_llnl_singlelat", 2, 4, 128, 2, 128, 1e-4, 12],
    # ["run_distributed_training.py", "train_lvae_dist_llnl_singlelat", 2, 4, 128, 2, 128, 1e-4, 1e-5],
    # ["run_distributed_training.py", "train_lvae_dist_llnl_singlelat", 2, 4, 128, 2, 128, 1e-4, 1e-8],
    # ["run_distributed_training.py", "train_lvae_dist_llnl_singlelat", 2, 4, 128, 2, 128, 1e-4, 1.0],
]
...
for exp in final_exp_list:
    ...
    # resume_from_path = f" resume_from=/p/vast1/madan2/diffusion-root/ldlm/outputs_killian/7ec84df29a-lvae_bart-gbz-1024-grad_accum-2-seq_len-128-n_lats-8-lat_dim-1536/7ec84df29a7eb0af9bc35f06d51c482f"
    cli_args += train_bsz_cfg_string

    # compute max steps automatically for token target
    max_steps = int(TGT_TOKENS / (wbsz*seq_len))+1
    cli_args += f" train_num_steps={max_steps} kld_weight={kld_weight}"
