...
class Trainer(object):
    ...
    def __init__(self,
        ...
        if args.freeze_bb == "freeze":
            print("Using Frozen BART Backbone")
            ctx = torch.no_grad()
        else:
            ctx = nullcontext()
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                with accelerator.autocast():
                    enc_outs, var_loss = self.model.bart_autoencode(data["input_ids"], attn_mask=data["attention_mask"])
                    model_loss = self.model(labels=data['labels'], encoder_outputs=enc_outs).loss

                    current_kld_weight = 1e-7 # min(1e-5, self.step / KLD_WARMUP_STEPS) 
