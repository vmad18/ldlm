...
class TrainingConfig:
    ...
    precomputed_latent_path: Optional[str] = None
...
def main(cfg: TrainingConfig):
    ...
    if master_process:
        ...
        # TODO: change the name for wandb based on n_lats, seq_len, etc
        wandb.init(
    ...
    if cfg.bb == "bart":
        ...
        model, tokenizer, config = get_latent_vae_tokenizer_bart(cfg, ctx, world_size)
        model: LatentVAEModel = model
        ...
        model, tokenizer, config = get_latent_vae_tokenizer_t5(cfg, ctx, world_size)
        model: LatentVAEModel = model
        # model = torch.compile(model, mode="max-autotune-no-cudagraphs")
    else:
    ...
    print(f"====> NUM MODEL PARAMS: {(sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6):.4f}M <====")
    ...
    # TODO: remove this Fasle right here 
    if False and cfg.resume_from is not None:
        if master_process:
    ...
    if cfg.resume_from is not None and step > 0:
        ...
        train_loader = get_dataloader(
                                        ...
                                        config, 
                                        tokenizer, 
                                        cfg.model_config.max_seq_len, 
                                        context_tokenizer = tokenizer,
                                        use_precomputed_latents = cfg.use_precomputed_latents,
                                        precomputed_latent_path = cfg.precomputed_latent_path,
        ...
        train_loader_shadow = cycle(train_loader)
        val_loader_shadow = cycle(val_loader)
    ...
    for _ in range(warmup_steps):
        batch = next(train_loader_shadow)
        batch = {k: v.cuda() for k, v in batch.items()}
        
        # batch["input_ids"], attn_mask=batch["attention_mask"]
        losses = model(**batch)
        loss = losses['lm_loss'] + get_annealed_kld_weight(0) * losses['vae_loss']['kld_loss']
        loss.backward()
    ...
    # TODO: optimizer_muon.load_state_dict(initial_state["optimizer_muon"])
    ...
    for step in range(step, cfg.train_num_steps + 1):
        ...
        if last_step or (cfg.eval_every > 0 and step % cfg.eval_every == 0):
            ...
            if master_process:
                ...
                print0(f"step:{step}/{cfg.train_num_steps} val_loss:{val_loss:.4f} "
                       f"val_lm_loss:{val_lm_loss:.4f} val_recon_loss:{val_recon_loss:.4f} val_kld_loss:{val_kld_loss:.4f} "
                ...
                # /p/vast1/madan2/diffusion-root/ldlm/outputs/5c30cbd9409ecfb222b6d08b1bfdf18d
                if step % (cfg.eval_every * 2) == 0:
                    # TODO: translate this for bb code
                    pass 
                    ...
                    gen_ids = model.decode_latent(latents, max_length = cfg.model_config.max_seq_len)
        ...
        # TODO # Muon optimizer - use cfg.muon_lr as base  
        # for group in optimizer_muon.param_groups:
        ...
        if step % cfg.log_step_interval == 0:
            ...
            print0(f"step:{step+1}/{cfg.train_num_steps} loss:{total_loss:.4f} "
                   f"lm_loss:{total_lm_loss:.4f} recon_loss:{total_recon_loss:.4f} kld_loss:{total_kld_loss:.4f} "
